...
#pragma region 描画初期化処理
	...
	IndexBuff index;
	index.Initialize(device);

	Pipeline pipeline;
	pipeline.Initialize(device,noBlend);
	...
	result = D3D12SerializeRootSignature(&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1_0,
		&rootSigBlob, &pipeline.errorBlob);
	assert(SUCCEEDED(result));
	...
	rootSigBlob->Release();
	// パイプラインにルートシグネチャをセット
	pipeline.pipelineDesc.pRootSignature = rootSignature;

	// パイプランステートの生成
	ID3D12PipelineState* pipelineState = nullptr;
	result = device->CreateGraphicsPipelineState(&pipeline.pipelineDesc, IID_PPV_ARGS(&pipelineState));
	assert(SUCCEEDED(result));
#pragma endregion
...
#pragma region DirectX毎フレーム処理
		...
		if (key[DIK_SPACE])
		{
			pipeline.Initialize(device, sub);
		}
...
#pragma region グラフィックスコマンド
		...
		// 頂点バッファビューの設定コマンド
		commandList->IASetVertexBuffers(0, 1, &index.vbView);

		//定数バッファビュー(CBV)の設定コマンド
		commandList->SetGraphicsRootConstantBufferView(0, constBuffMaterial->GetGPUVirtualAddress());

		// 描画コマンド
		commandList->DrawInstanced(index.vertices.size(), 1, 0, 0); // 全ての頂点を使って描画
		// 4.描画コマンドここまで
